...
int main() {
  ...
  //[&image_table] 这是 lambda 表达式的特性, 捕获变量
  ...
  server.Post("/image", [&image_table](const Request& req, Response& resp) 
      ...
      MD5((unsigned char *)body.c_str(), body.length(), mdStr);
      for(int i = 0; i < 16; i++){                                                                 
        sprintf (tmp,"%2.2x",mdStr[i]);
        strcat (buf,tmp);
      }
      //获取当前系统时间
      timespec time;
      ...
      char current[1024];
      sprintf(current, "%04d%02d%02d%02d:%02d:%02d", nowTime.tm_year + 1900, 
          nowTime.tm_mon+1, nowTime.tm_mday, 
          nowTime.tm_hour, nowTime.tm_min, nowTime.tm_sec);
      Json::Value image;
      image["image_name"] = file.filename;
      image["type"] = file.content_type;
      image["md5"] = std::string((char*)buf).c_str();
      image["path"] = "./data/" + file.filename;
      image["upload_time"] = std::string(current).c_str();
